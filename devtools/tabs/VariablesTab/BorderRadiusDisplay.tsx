...
/**
 * BorderRadiusDisplay - Read-only display of border radius values from globals.css
 */
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        {RADIUS_KEYS.map((key) => (
          <div key={key} className="flex items-center gap-2 p-2 bg-black/10 rounded-sm">
            {/* Visual Preview */}
            <div
              className="w-8 h-8 bg-green border border-black flex-shrink-0"
              style={{ borderRadius: borderRadius[key] || '0' }}
            />
            
            {/* Key and Value */}
            <div className="flex-1 min-w-0">
              <span className="font-alfacad text-base font-mono text-black block">
                {key}
              </span>
      ...
      <div className="p-3 bg-green/20 border border-black/20 rounded-sm">
        <p className="font-alfacad text-sm text-black">
          Border radius values are display-only. Edit values directly in globals.css using the Cursor visual editor.
        </p>
      </div>
    </div>
  );

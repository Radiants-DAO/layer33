...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-6">
      <h3 className="font-space-mono text-xs uppercase text-black mb-3 border-b border-black/20 pb-2">
        {title}
      </h3>
...
If the preview file doesn't exist, create it with the necessary imports.`;
  ...
  return (
    <div className="border border-black bg-white -mt-px first:mt-0">
      <div className="overflow-hidden transition-[height] duration-200 ease-out">
        <div className="px-4 pb-4">
          <div className="space-y-6">
            <div className="mb-6">
              <div className="flex items-center justify-between mb-3">
                <h4 className="font-space-mono text-xs uppercase text-black">{component.name}</h4>
                <Button
                  variant="outline"
                  size="sm"
              ...
              <div className="space-y-2">
                <div className="font-alfacad text-sm text-black/60 font-mono">
                  {component.path}
                </div>
...
export function ComponentList({ components, folderName }: ComponentListProps) {
  ...
  const [previewedComponents, setPreviewedComponents] = useState<Set<string>>(new Set());

  // Query DOM for components that already have previews rendered
  useEffect(() => {
    const queryPreviewedComponents = () => {
      ...
      setPreviewedComponents(names);
    };

    // Initial query
    queryPreviewedComponents();

    // Set up a MutationObserver to watch for DOM changes
    const observer = new MutationObserver(queryPreviewedComponents);
  ...
  // Filter out components that already have previews, then apply search filter
  const filtered = components
    .filter((c) => !previewedComponents.has(c.name))
    .filter((c) =>

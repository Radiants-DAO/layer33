...
export function TokenDropdown({ value, onChange, label }: TokenDropdownProps) {
  ...
  return (
    <div className="flex items-center gap-2">
      {label && (
        <label className="font-space-mono text-xs uppercase text-black/60 min-w-[60px]">{label}</label>
      )}
      <select
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="flex-1 px-2 py-1.5 font-alfacad text-base bg-white border border-black rounded-sm text-black focus:outline-none focus:ring-2 focus:ring-tertiary cursor-pointer"
      >
